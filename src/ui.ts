...
function logo(): string {
  const c = (s: string, color: (t: string) => string) => color(s);
  const block = (char: string, col: (t: string) => string) => col(char);
...
export const ui = {
  ...
  shortcutsLine: (left: string = 'shift+tab to accept edits', right: string = '? for shortcuts', center?: string) => {
