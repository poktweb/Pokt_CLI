...
import {
  runMcpFromBashMarkdown,
  stripExecutedStyleMcpBashBlocks,
  tryAutoMcpForListDatabases,
} from './mcp-from-text.js';
...
function buildSystemPrompt(mcpToolNames: string[]): string {
  if (mcpToolNames.length === 0) return SYSTEM_PROMPT_BASE;
...
function suggestsProjectModification(userInput: string): boolean {
...
function extractPyFilesFromStructure(structure: string): string[] {
...
function inferTargetFileFromProject(projectStructure: string, userInput: string): string | null {
...
export async function startChatLoop(modelConfig: ModelConfig) {
  ...
  const toolsForApi: ChatCompletionTool[] =
    activeModel.provider === 'controller' ? slimToolsForUpstreamPayload(allTools) : allTools;

  const mcpToolNames = allTools
    .filter((t): t is ChatCompletionTool & { function: { name: string } } => t.type === 'function' && Boolean(t.function?.name?.startsWith('mcp_')))
    .map((t) => t.function.name);

  const messages: ChatCompletionMessageParam[] = [{ role: 'system', content: buildSystemPrompt(mcpToolNames) }];
...
`)
  ...
  const sessionFileCtx: SessionFileToolContext = {
...
`)
    ...
    if (needsProjectContext) {
      ...
      let structureHint =
        '[Pokt] Use list_files/read_file para confirmar arquivos existentes. Para modificar: leia com read_file, edite com search_replace ou write_file no MESMO caminho — NÃO crie main.py ou novo projeto.';
      if (targetFile) {
...
function getStatusCode(err: any): number | null {
...
function isShellLikeBlock(lang: string): boolean {
...
function isFallbackPathBlocked(relPath: string): boolean {
...
function looksLikeMcpOrSqlResultSnippet(code: string): boolean {
  const t = code.trim();
  if (!t) return false;
  if (/MCP error|"invalid_type"|Input validation error/i.test(t)) return true;
  try {
    const j = JSON.parse(t) as unknown;
    if (j !== null && typeof j === 'object' && !Array.isArray(j)) {
      const o = j as Record<string, unknown>;
      if ('success' in o && 'result' in o) return true;
      if (typeof o.error === 'string' && /mcp|tool|validation|invalid/i.test(o.error)) return true;
    }
    if (Array.isArray(j) && j.length > 0 && j.length <= 200) {
      ...
      if (first && typeof first === 'object' && !Array.isArray(first)) {
        ...
        if (!keys.every((k) => /^[a-zA-Z_][a-zA-Z0-9_]*$/.test(k))) return false;
...
function looksLikeFakeMcpInvocation(code: string): boolean {
...
type ChatCompletionAssistantMessage = ChatCompletion['choices'][0]['message'];

/** Contexto mutável: último arquivo tocado + arquivo inferido para edição em pedidos de modificação. */
type SessionFileToolContext = {
  lastFocusedRelativePath: string | null;
  /** Arquivo existente inferido da estrutura (ex: calculadora.py) para editar em vez de criar main.py */
  suggestedModificationTarget: string | null;
};

function getFirstChoiceMessage(completion: ChatCompletion): ChatCompletionAssistantMessage | null {
  const msg = completion.choices?.[0]?.message;
  return msg ?? null;
}

function trackToolPathFocus(name: string, argsStr: string, ctx: SessionFileToolContext): void {
  if (name !== 'read_file' && name !== 'write_file' && name !== 'search_replace') return;
  try {
    const args = JSON.parse(argsStr) as { path?: unknown };
 ...
 */
function preferSessionPathOverGenerated(
  resolvedPath: string,
  ctx: SessionFileToolContext | null
): string {
  ...
  if (!sessionLast || !/^generated\./i.test(resolvedPath)) return resolvedPath;
  const suffix = resolvedPath.slice(resolvedPath.indexOf('.') + 1).toLowerCase();
  const normLast = sessionLast.replace(/\\/g, '/').toLowerCase();

  const extensionsForSuffix: Record<string, readonly string[]> = {
    py: ['.py'],
    python: ['.py'],
    js: ['.js'],
    javascript: ['.js'],
  ...
  if (exts?.some((e) => normLast.endsWith(e))) return sessionLast.replace(/\\/g, '/');
  if (normLast.endsWith('.' + suffix)) return sessionLast.replace(/\\/g, '/');
  return resolvedPath;
}

async function applyCodeBlocksFromContent(
  content: string,
  sessionFileCtx: SessionFileToolContext | null
): Promise<{ applied: boolean; displayContent: string }> {
  const codeBlockRe = /```(\w*)\n([\s\S]*?)```/g;
  ...
  for (const { fullMatch, index, lang, code } of matches) {
    if (isShellLikeBlock(lang)) continue;
    if (looksLikeFakeMcpInvocation(code)) continue;
    ...
    let path =
      candidate && CODE_EXT.test(candidate) ? candidate : lang === 'python' ? 'generated.py' : lang ? `generated.${lang}` : null;
    if (path) {
      path = preferSessionPathOverGenerated(path, sessionFileCtx);
    }
    if (path && isFallbackPathBlocked(path)) continue;
    if (
      path &&
    ...
    if (path && code) {
      try {
        ...
        applied = true;
        if (sessionFileCtx) sessionFileCtx.lastFocusedRelativePath = path.replace(/\\/g, '/');
...
async function createCompletionWithRetry(
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (usageAcc) mergeCompletionUsage(usageAcc, out);
      return out;
    } catch (err: any) {
      lastError = err;
...
async function drainToolCalls(
  ...
  messages: ChatCompletionMessageParam[],
  toolsList: ChatCompletionTool[],
  startMessage: ChatCompletionAssistantMessage,
  spinner: ReturnType<typeof ora>,
  sessionFileCtx: SessionFileToolContext,
  usageAcc?: ChatUsageAccumulator
): Promise<{
  message: ChatCompletionAssistantMessage;
  writeFileExecuted: boolean;
  anyToolExecuted: boolean;
  mcpToolExecuted: boolean;
}> {
  ...
  while (message.tool_calls && message.tool_calls.length > 0) {
    ...
    for (const toolCall of message.tool_calls) {
      ...
      if (name === 'write_file' || name === 'search_replace') writeFileExecuted = true;
      if (isMcpTool(name)) mcpToolExecuted = true;
      ...
      if (isMcp && !verbose) {
        ...
        console.log(ui.warn(`\n[Executing Tool: ${name}]`));
        if (verbose || !isMcp) console.log(ui.dim(`Arguments: ${args}`));
      }
...
function emitCliTelemetryForTurn(modelConfig: ModelConfig, acc: ChatUsageAccumulator): void {
...
async function processLLMResponse(
  ...
  try {
    ...
    const drained = await drainToolCalls(
      client,
      modelId,
      messages,
      toolsList,
    ...
    let finalContent: string | typeof rawContent = rawContent ?? contentStr;
    ...
    if (!contentStr.trim() && toolsList.length > 0) {
      ...
      const drained2 = await drainToolCalls(
        client,
        modelId,
        messages,
        toolsList,
    ...
    if (status === 429) {
      ...
      console.log(
        ui.dim(
          'Dica: use /chat save e /clear ou comece sessão nova; histórico + ferramentas MCP (Neon) estouram o limite do servidor. O Pokt já reduz schemas ao usar o Controller — atualize o Controller (limite JSON) e o CLI.'
        )
      );
    } else {
