...
async function drainToolCalls(
  ...
  while (message.tool_calls && message.tool_calls.length > 0) {
    ...
    for (const toolCall of message.tool_calls) {
      ...
      if (name === 'write_file') writeFileExecuted = true;
  ...
  try {
    ...
    if (!contentStr.trim() && toolsList.length > 0) {
      messages.push({
        role: 'system',
        content:
          '[Pokt — recuperação] A última resposta veio vazia (sem texto útil). Responda em português. ' +
    ...
    if (!writeFileExecutedThisTurn) {
      ...
      if (contentStr.trim() !== '') {
        ...
        console.log(
          ui.dim(
            '(Sem resposta da IA após recuperação. Tente: outro modelo em /model, ou peça explicitamente “chame mcp_Neon_run_sql com SELECT datname FROM pg_database…”, ou use run_command com psql/node.)'
          )
        );
        messages.push({ role: 'assistant', content: '' });
      }
