...
async function checkModelsEndpoint(client: Awaited<ReturnType<typeof getClient>>): Promise<number> {
...
async function checkOllamaTags(): Promise<number> {
...
export const doctorCommand: Yargs.CommandModule = {
  ...
  handler: async () => {
    ...
    const label = PROVIDER_LABELS[active.provider as Provider] ?? active.provider;
    console.log(ui.dim(`\nProvider ativo: ${label}`));
    console.log(ui.dim(`Model ativo: ${active.id}\n`));

    // 1) checar credenciais (env ou conf)
    const required: Record<Provider, { name: string; value: string; hint?: string } | null> = {
    ...
    const req = required[active.provider as Provider] ?? null;
    if (req) {
      if (!req.value) {
