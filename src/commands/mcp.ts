...
function tagSource(s: McpServerConfig): string {
...
export const mcpCommand: Yargs.CommandModule<{}, McpArgs> = {
  ...
  handler: async (argv: McpArgs) => {
    ...
    const globalServers: McpServerConfig[] = config.get('mcpServers') ?? [];
    ...
    if (action === 'list') {
      ...
      if (merged.length === 0) {
        console.log(
          ui.dim(
            'Nenhum servidor MCP. Adicione no projeto (pokt_cli/mcp.json) ou global: pokt mcp add -n <name> -t stdio -c npx -a \'["-y","mcp-server"]\''
          )
        );
        return;
      }
    ...
    if (action === 'add') {
      ...
      if (!name?.trim()) {
        console.log(
          ui.error(
            '--name é obrigatório. Ex.: pokt mcp add -n filesystem -t stdio -c npx -a \'["-y","@modelcontextprotocol/server-filesystem"]\''
          )
        );
        return;
      }
